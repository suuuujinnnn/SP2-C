...
void setup() {
  ...
  myservo.writeMicroseconds(_DUTY_MIN); // initialize servo at 0 degrees
...
void loop() {
  ...
  // Range filter: restrict measured distance to be within _DIST_MIN and _DIST_MAX
  ...
  if (dist_ema <= 180.0) {
    ...
    servo_angle = map(dist_ema, 180, 360, 0, 180);
  } else {
    servo_angle = 180;
  }
  ...
  // Output the distance and servo data to the serial port
